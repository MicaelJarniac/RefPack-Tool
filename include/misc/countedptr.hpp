 ...
 */
struct ICountedObject {
	virtual int32 AddRef() = 0;
	virtual int32 Release() = 0;
	virtual void FinalRelease() {};
};

template <class Tbase = ICountedObject> struct CountedObjectImplT : public Tbase {
	int32 m_ref_cnt;
...
template <class Tbase> inline int32 CountedObjectImplT<Tbase>::AddRef()
{
	return ++m_ref_cnt;
}

template <class Tbase> inline int32 CountedObjectImplT<Tbase>::Release()
{
	int32 res = --m_ref_cnt;
	assert(res >= 0);
	if (res == 0) {
